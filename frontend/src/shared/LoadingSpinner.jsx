...
export default function LoadingSpinner() {
      ...
      return (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center   backdrop-blur-sm px-6 text-center space-y-3">
...
export function OverlaySpinner({ size = 4, message = null }) {
      const { theme } = useTheme();
            ...
            <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                  <div className="flex flex-col items-center space-y-4 bg-white dark:bg-stone-800 px-8 py-6 rounded-2xl shadow-xl">
                        ...
                        <div className="flex space-x-2">
                              <span className={`rounded-full ${theme.color} h-${size} w-${size} animate-wave delay-0`} />
                              <span className={`rounded-full ${theme.color} h-${size} w-${size} animate-wave delay-200`} />
                              <span className={`rounded-full ${theme.color} h-${size} w-${size} animate-wave delay-400`} />
                        </div>
