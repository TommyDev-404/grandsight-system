                                    ...
                                    {areaDetails.map(data => (
                                          <tr key={data.room} className="bg-stone-50 dark:bg-stone-800 border-b border-stone-200 dark:border-stone-700 hover:bg-stone-100 dark:hover:bg-stone-700 text-stone-800 dark:text-white">
                                                ...
                                                <td className="py-3 flex justify-center">
                                                      <button onClick={completeStatusName(data.status) === 'Ready/Available' || completeStatusName(data.status) === 'Occupied' ? () => handleOpenIndividualCleaning(data.room) : completeStatusName(data.status) === 'Need-Clean' ? () => handleAssignCleaner(data.room) : () => handleMarkReady(data.room) } className={` p-2 rounded-full  ${completeStatusName(data.status) === 'Ready/Available' || completeStatusName(data.status) === 'Occupied' ? 'bg-purple-500 hover:bg-purple-600' : completeStatusName(data.status) === 'Need-Clean' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}>
